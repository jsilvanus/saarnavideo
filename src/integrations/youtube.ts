...
export async function uploadToYouTube(input: YouTubeUpload): Promise<{ videoId: string }> {
  ...
  if (input.thumbnailPath) { const thumbnail = await readFile(input.thumbnailPath); const thumb = await fetch(`https://www.googleapis.com/upload/youtube/v3/thumbnails/set?videoId=${encodeURIComponent(result.id)}`, { method: "POST", headers: { Authorization: `Bearer ${input.accessToken}`, "Content-Type": "image/jpeg", "Content-Length": String(thumbnail.length) }, body: thumbnail }); if (!thumb.ok) throw new Error(`YouTube thumbnail upload failed (${thumb.status}): ${await thumb.text()}`); }
