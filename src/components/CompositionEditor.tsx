...
export default function CompositionEditor({ definition, sources, onChange }: Props) {
  ...
  const updateItem = async (index: number, patch: Partial<Item>) => commit(items.map((item, i) => i === index ? { ...item, ...patch } : item));
    ...
    <div className="composition-timeline">
      ...
      {items.map((item, itemIndex) => {
        ...
        return <div key={`section-${section.id}`} className="composition-section">
          <div className="section-header"><div className="section-thumbnail"><SectionThumbnail source={sources.find(s => s.id === section.sourceId)} projectSectionStart={section.startSeconds} /></div><div><strong>{section.label}</strong><small>{formatTime(section.startSeconds)} → {formatTime(section.endSeconds)} · {sourceName(sources, section.sourceId)}</small></div></div>
