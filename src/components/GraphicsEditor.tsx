...
function defaultLayers(item: Item, title: string): Layer[] {
  ...
  if (stored) { try { const parsed = JSON.parse(stored); if (Array.isArray(parsed)) return parsed; } catch { /* old/simple graphic */ } }
...
export default function GraphicsEditor({ projectId, item, assets, title, onChange }: { projectId: string; item: Item; assets: Asset[]; title: string; onChange: (item: Item) => void }) {
  ...
  const [grid, setGrid] = useState(false); const [safe, setSafe] = useState(false); const [aspectLock, setAspectLock] = useState(true);
  const [history, setHistory] = useState<Layer[][]>([]); const [historyIndex, setHistoryIndex] = useState(-1); const [assetPicker, setAssetPicker] = useState(false);
  const canvasRef = useRef<HTMLDivElement>(null); const artboardRef = useRef<HTMLDivElement>(null); const dragRef = useRef<any>(null); const layersRef = useRef(layers); layersRef.current = layers;

  useEffect(() => { setLocalAssets(assets); }, [assets]);
  useEffect(() => { const next = defaultLayers(item, title); setLayers(next); setSelectedIds(next[0] ? new Set([next[0].id]) : new Set()); setPrimaryId(next[0]?.id ?? null); }, [item, title]);

  const pushHistory = (next: Layer[]) => { setHistory(prev => [...prev.slice(0, historyIndex + 1), layersRef.current].slice(-50)); setHistoryIndex(prev => Math.min(prev + 1, 49)); setLayers(next); onChange({ ...item, template: "rich", data: { ...(item.data ?? {}), layers: serialiseLayers(next) } }); };
  function updateLayer(id: string, patch: Partial<Layer>) { pushHistory(layers.map(l => l.id === id ? { ...l, ...patch } : l)); }
  ...
  function addLayer(type: Layer["type"]) {
    ...
    const base: Layer = type === "text" ? { id, type, x: 220, y: 360, width: 1480, height: 160, text: "Text", style: { "font-size": "72px", color: "#ffffff", "font-weight": "700", "text-align": "center" } } : type === "image" || type === "svg" ? { id, type, x: 460, y: 300, width: 1000, height: 560 } : { id, type, x: 460, y: 320, width: 1000, height: 440, style: { background: type === "ellipse" ? "#ffffff" : "#000000" } };
    pushHistory([...layers, base]); setSelectedIds(new Set([id])); setPrimaryId(id); if (type === "image" || type === "svg") setAssetPicker(true);
  }
  ...
  function beginPointer(e: React.PointerEvent, layerId: string, kind: string, handle?: string) {
    e.stopPropagation(); const layer = layersRef.current.find(l => l.id === layerId); if (!layer) return; const rect = artboardRef.current?.getBoundingClientRect(); if (!rect || !rect.width) return; const scale = rect.width / WIDTH;
    ...
    dragRef.current = { kind, layerId, handle, startX: e.clientX, startY: e.clientY, layer: { ...layer }, scale, artboardLeft: rect.left, artboardTop: rect.top }; (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);
  }
  function pointerMove(e: React.PointerEvent) {
    const d = dragRef.current; if (!d) return; const dx = (e.clientX - d.startX) / d.scale, dy = (e.clientY - d.startY) / d.scale; if (Math.abs(dx) < 1 && Math.abs(dy) < 1) return; const next = layersRef.current.map(l => ({ ...l })); const idx = next.findIndex(l => l.id === d.layerId); if (idx < 0) return;
  ...
  function chooseAsset(asset: Asset) {
    if (!primary || (primary.type !== "image" && primary.type !== "svg")) return;
    const layerType: Layer["type"] = asset.mimeType === "image/svg+xml" ? "svg" : "image";
    updateLayer(primary.id, { type: layerType, assetId: asset.id, src: `/api/projects/${projectId}/assets/${asset.id}` }); setAssetPicker(false);
  }
  async function uploadAsset(file: File) {
    const assetKey = `${file.name.replace(/\.[^.]+$/, "").toLowerCase().replace(/[^a-z0-9_-]+/g, "-").slice(0, 48)}-${Date.now()}`;
    const form = new FormData(); form.append("file", file); form.append("assetKey", assetKey); form.append("type", file.type === "image/svg+xml" || file.name.toLowerCase().endsWith(".svg") ? "LOGO" : "OVERLAY");
    const response = await fetch(`/api/projects/${projectId}/assets`, { method: "POST", body: form }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Asset upload failed");
    setLocalAssets(prev => [data, ...prev.filter(a => a.id !== data.id)]); if (primary && (primary.type === "image" || primary.type === "svg")) chooseAsset(data);
  }
  const assetCount = useMemo(() => localAssets.filter(a => a.mimeType !== "font/ttf").length, [localAssets]);
