...
export default function GraphicsEditor({ projectId, graphicId, item, assets, title, onChange }: { projectId: string; graphicId: string; item: Item; assets: Asset[]; title: string; onChange: (item: Item) => void }) {
  ...
  return <div className="graphics-editor">
    ...
    <div className="ge-layout"><GraphicsEditorCanvas canvasRef={canvasRef} artboardRef={artboardRef} layers={layers} selectedIds={selectedIds} grid={grid} safe={safe} background={item.data?.backgroundColor ?? "#111"} onPointerMove={pointerMove} onPointerUp={pointerUp} onCanvasPointerDown={() => { setSelectedIds(new Set()); setPrimaryId(null); }} onLayerPointerDown={beginPointer} /><GraphicsEditorProperties projectId={projectId} graphicId={graphicId} item={item} assets={assets} primary={primary} assetPicker={assetPicker} aspectLock={aspectLock} onLayer={updateLayer} onStyle={updateStyle} onChooseAsset={chooseAsset} onToggleAssetPicker={() => setAssetPicker(v => !v)} onAspectLock={setAspectLock} /></div>
