...
export const GraphicsEditorProperties: FC<{
  projectId: string; item: Item; assets: Asset[]; primary: Layer | null; assetPicker: boolean; aspectLock: boolean;
  onLayer: (id: string, patch: Partial<Layer>) => void; onStyle: (id: string, key: string, value: string | number) => void;
  onChooseAsset: (asset: Asset) => void; onToggleAssetPicker: () => void; onUploadAsset: (file: File) => Promise<void>;
  onAspectLock: (value: boolean) => void;
}> = ({ projectId: _projectId, item: _item, assets, primary, assetPicker, aspectLock, onLayer, onStyle, onChooseAsset, onToggleAssetPicker, onUploadAsset, onAspectLock }) => (
  <aside className="ge-properties">
    {primary && <div className="ge-section"><b>Layer: {primary.id}</b><label>Type<span>{primary.type}</span></label>
      {primary.type === "text" && <label>Text<textarea value={primary.text ?? ""} onChange={e => onLayer(primary.id, { text: e.target.value })} /></label>}
      {(primary.type === "image" || primary.type === "svg") && <label>{primary.type === "svg" ? "SVG asset" : "Image asset"}<button onClick={onToggleAssetPicker}>{primary.assetId ? "Change asset" : "Choose asset"}</button></label>}
      ...
      {primary.type === "text" && <><label>Font family<input value={styleValue(primary, "font-family", "Arial, sans-serif")} onChange={e => onStyle(primary.id, "font-family", e.target.value)} /></label><label>Font size<input value={styleValue(primary, "font-size", "72px")} onChange={e => onStyle(primary.id, "font-size", e.target.value)} /></label><div className="ge-two"><label>Weight<select value={styleValue(primary, "font-weight", "700")} onChange={e => onStyle(primary.id, "font-weight", e.target.value)}><option>normal</option><option>bold</option><option>400</option><option>500</option><option>600</option><option>700</option><option>800</option><option>900</option></select></label><label>Align<select value={styleValue(primary, "text-align", "center")} onChange={e => onStyle(primary.id, "text-align", e.target.value)}><option>left</option><option>center</option><option>right</option></select></label></div><label>Color<input type="text" value={styleValue(primary, "color", "#fff")} onChange={e => onStyle(primary.id, "color", e.target.value)} /></label><label>Text shadow<input value={styleValue(primary, "text-shadow")} onChange={e => onStyle(primary.id, "text-shadow", e.target.value)} placeholder="0 3px 10px #000" /></label><label>Text stroke<input value={styleValue(primary, "-webkit-text-stroke")} onChange={e => onStyle(primary.id, "-webkit-text-stroke", e.target.value)} placeholder="1px #000" /></label></>}
    ...
    {assetPicker && <div className="ge-section"><b>Asset gallery</b><label className="ge-upload">Upload SVG/image<input type="file" accept=".svg,image/png,image/jpeg,image/webp" onChange={e => { const file = e.target.files?.[0]; if (file) void onUploadAsset(file); e.currentTarget.value = ""; }} /></label><div className="ge-asset-grid">{assets.filter(a => a.type !== "FONT").map(a => <button key={a.id} className="ge-asset-tile" onClick={() => onChooseAsset(a)}><span>{a.mimeType === "image/svg+xml" ? "SVG" : "IMG"}</span><strong>{a.assetKey}</strong></button>)}</div>{!assets.length && <span>No assets yet.</span>}</div>}
  </aside>
);
