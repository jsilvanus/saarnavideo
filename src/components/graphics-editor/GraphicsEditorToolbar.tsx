...
export const GraphicsEditorToolbar: FC<{
  ...
  return <div className="ge-toolbar" aria-label="Graphic tools">
    ...
    <button onClick={onExportEmbedded}>Export .svgraphic</button>
    <button onClick={onExportReferenced}>Export refs</button>
    <button onClick={() => document.getElementById(inputId)?.click()}>Import .svgraphic</button>
    <input id={inputId} type="file" accept=".svgraphic,application/vnd.saarnavideo.graphic+json,application/json" hidden onChange={e => { const file = e.target.files?.[0]; if (file) onImport(file); e.currentTarget.value = ""; }} />
