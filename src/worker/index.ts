...
async function logJobEvent(jobId: string, level: "DEBUG" | "INFO" | "WARN" | "ERROR", message: string, data?: Record<string, unknown>) {
  try {
    await prisma.jobLog.create({ data: { jobId, level, message, data: data ? JSON.stringify(data) : undefined } });
  } catch (error) {
...
async function createOutput(projectId: string, jobId: string, type: "VIDEO" | "THUMBNAIL", storagePath: string, mimeType: string) {
  ...
  try {
    ...
    if (sizeBytes > maxSize) throw new Error(`${type} file size ${formatBytes(sizeBytes)} exceeds limit ${formatBytes(maxSize)}`);
  } catch (error) {
...
async function acquireSource(jobId: string, project: { id: string; definition: unknown; sources: Array<{ id: string; type: "UPLOAD" | "YOUTUBE"; storagePath: string | null; youtubeVideoId: string | null; youtubeUrl: string | null }> }): Promise<Map<string, string>> {
  ...
  for (const source of project.sources) {
    ...
    if (source.storagePath) {
      try {
        ...
        await logJobEvent(jobId, "INFO", "Using existing source file", { sourceId: source.id, path: source.storagePath, sizeBytes: stats.size });
        sourcePaths.set(source.id, source.storagePath);
        continue;
      } catch (error) {
        await logJobEvent(jobId, "WARN", "Existing source file unavailable, will retry acquisition", { sourceId: source.id, error: error instanceof Error ? error.message : String(error) });
      }
    }
    if (source.type === "YOUTUBE") {
      if (!source.youtubeUrl || !source.youtubeVideoId) throw new Error(`YouTube source missing URL or video ID for sourceId: ${source.id}`);
      if (await checkCancellation(jobId)) throw new Error("Job cancelled during source acquisition");
      const directory = path.join(MEDIA_ROOT, "sources", project.id);
      const storagePath = path.join(directory, `${source.youtubeVideoId}.mp4`);
      await logJobEvent(jobId, "INFO", "Starting YouTube source download", { sourceId: source.id, videoId: source.youtubeVideoId, url: source.youtubeUrl });
      try {
        ...
        const validation = validateSourceFile(stats.size, RESOURCE_LIMITS);
        if (!validation.valid) { await rm(storagePath, { force: true }).catch(() => undefined); throw new Error(validation.reason); }
        await prisma.source.update({ where: { id: source.id }, data: { storagePath, mimeType: "video/mp4", sizeBytes: BigInt(stats.size), expiresAt: new Date(Date.now() + RETENTION_MS) } });
        ...
        await logJobEvent(jobId, "ERROR", "YouTube source download failed", { sourceId: source.id, error: error instanceof Error ? error.message : String(error) });
        throw error;
      }
      continue;
    }
  ...
  return resolveSourcePaths(definition, project.sources.map(source => ({ ...source, storagePath: sourcePaths.get(source.id) ?? source.storagePath })));
}
...
async function processPublication() {
  ...
  if (output.expiresAt && output.expiresAt <= new Date()) {
    await prisma.publication.update({ where: { id: publication.id }, data: { status: "FAILED", error: "Output has expired" } });
    return true;
  }
...
async function processJob() {
  ...
  try {
    ...
    if (await checkCancellation(job.id)) {
      await prisma.generationJob.update({ where: { id: job.id }, data: { status: "CANCELLED", cancelledAt: new Date() } });
      await logJobEvent(job.id, "INFO", "Job cancelled by user");
      return true;
    }

    // A generation with explicit EOF clamping uses an immutable render snapshot.
    // The saved project definition remains untouched, so the user's original timestamps are preserved.
    const definition = (job.renderDefinition ?? project.definition) as unknown as ProjectDefinition;
    const renderProject = { ...project, definition };
    const sourcePaths = await acquireSource(job.id, renderProject);

    await prisma.generationJob.update({ where: { id: job.id }, data: { status: "PROCESSING", progress: 25 } });
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ffmpegProcess.on("close", code => { runningProcesses.delete(job.id); if (code === 0) resolve(); else reject(new Error(`FFmpeg exited with code ${code}`)); });
      ...
      if (ffmpegProcess && !ffmpegProcess.killed) { ffmpegProcess.kill("SIGTERM"); runningProcesses.delete(job.id); }
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    await prisma.generationJob.update({ where: { id: job.id }, data: { status: "FAILED", error: errorMessage } });
...
async function cleanupExpiredMedia() {
  ...
  let deletedSources = 0; let deletedOutputs = 0;
  ...
  if (deletedSources > 0 || deletedOutputs > 0) console.log(`Cleanup: removed ${deletedSources} expired sources, ${deletedOutputs} expired outputs`);
}

process.on("SIGTERM", async () => {
  console.log("Received SIGTERM, shutting down gracefully...");
  for (const [jobId, proc] of runningProcesses) { console.log(`Terminating FFmpeg for job ${jobId}`); proc.kill("SIGTERM"); }
  process.exit(0);
});
