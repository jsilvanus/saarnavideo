...
type JobType = "DOWNLOAD" | "THUMBNAIL" | "PREVIEW" | "VIDEO";

async function logJobEvent(jobId: string, level: "DEBUG" | "INFO" | "WARN" | "ERROR", message: string, data?: Record<string, unknown>) {
  try { await prisma.jobLog.create({ data: { jobId, level, message, data: data ? JSON.stringify(data) : undefined } }); } catch (error) { console.error(`Failed to log job event: ${error}`); }
}

async function updateProgress(jobId: string, data: Record<string, unknown>, force = false) {
  const write = async () => { progressTimers.delete(jobId); try { await prisma.mediaJob.update({ where: { id: jobId }, data }); } catch (error) { console.error(`Failed to update job progress: ${error}`); } };
  if (force) { const timer = progressTimers.get(jobId); if (timer) clearTimeout(timer); await write(); return; }
  if (progressTimers.has(jobId)) return;
  progressTimers.set(jobId, setTimeout(() => void write(), PROGRESS_WRITE_MS));
}
...
async function claimJob() {
  ...
  for (const candidate of candidates) {
    if (candidate.dependsOnJobId) {
      ...
      if (dependency && dependency.status !== "COMPLETED") { if (dependency.status === "FAILED" || dependency.status === "CANCELLED") await prisma.mediaJob.update({ where: { id: candidate.id }, data: { status: "FAILED", error: "Dependency did not complete" } }); continue; }
    }
...
async function createOutput(projectId: string, jobId: string, type: "VIDEO" | "THUMBNAIL", storagePath: string, mimeType: string, preview = false) {
  ...
  try { sizeBytes = (await stat(storagePath)).size; const maxSize = type === "VIDEO" ? RESOURCE_LIMITS.maxOutputFileSizeBytes : 50 * 1024 * 1024; if (sizeBytes > maxSize) throw new Error(`${type} file size ${formatBytes(sizeBytes)} exceeds limit ${formatBytes(maxSize)}`); } catch (error) { if (error instanceof Error && error.message.includes("exceeds limit")) throw error; }
...
function sourcePathsFor(definition: ProjectDefinition, sources: Array<{ id: string; storagePath: string | null; type: "UPLOAD" | "YOUTUBE"; youtubeVideoId: string | null; youtubeUrl: string | null }>) {
  return resolveSourcePaths(definition, sources);
}

async function runFfmpegJob(job: Awaited<ReturnType<typeof claimJob>>, type: "VIDEO" | "PREVIEW" | "THUMBNAIL") {
  if (!job) throw new Error("Missing job");
  const project = await prisma.project.findUnique({ where: { id: job.projectId }, include: { sources: true, assets: true } });
  ...
  const sourcePaths = sourcePathsFor(definition, project.sources);
  const assetPaths = new Map<string, string>(); for (const asset of project.assets) { assetPaths.set(asset.assetKey, asset.storagePath); assetPaths.set(asset.id, asset.storagePath); assetPaths.set(`/api/projects/${project.id}/assets/${asset.id}`, asset.storagePath); }
  await mkdir(MEDIA_ROOT, { recursive: true });
  let outputPath = path.join(MEDIA_ROOT, `${project.id}-${job.id}${type === "PREVIEW" ? ".preview" : ""}.mp4`);
  if (type === "THUMBNAIL") {
    const video = await prisma.output.findFirst({ where: { projectId: project.id, type: "VIDEO", preview: false }, orderBy: { createdAt: "desc" } });
    if (!video) throw new Error("No completed video available for thumbnail");
    outputPath = path.join(MEDIA_ROOT, `${project.id}-${job.id}.jpg`);
    await updateProgress(job.id, { phase: "THUMBNAIL", message: "Extracting thumbnail", progress: 10 }, true);
    await execFileAsync("ffmpeg", ["-hide_banner", "-y", "-ss", "1", "-i", video.storagePath, "-frames:v", "1", "-q:v", "2", outputPath]);
    await createOutput(project.id, job.id, "THUMBNAIL", outputPath, "image/jpeg");
    await updateProgress(job.id, { status: "COMPLETED", progress: 100, phase: "COMPLETE", message: "Thumbnail ready", completedAt: new Date() }, true);
    return;
  }
  const plan = buildCompositionRenderPlan(definition, sourcePaths, outputPath, assetPaths);
  const outputIndex = plan.args.length - 1;
  if (type === "PREVIEW") plan.args.splice(outputIndex, 0, "-vf", "scale=640:-2", "-preset", "ultrafast", "-crf", "30");
  const totalMs = Math.max(1, Math.round((definition.composition.sourceEndSeconds - definition.composition.sourceStartSeconds) * 1000));
  plan.args.splice(plan.args.length - 1, 0, "-progress", "pipe:1", "-nostats");
  ...
  const progressListener = async (line: string) => { const values = Object.fromEntries(line.split("\n").map(x => x.split("=")).filter(x => x.length === 2)); if (values.out_time_ms) { const currentMs = Number(values.out_time_ms) / 1000; const progress = Math.max(0, Math.min(99, Math.round((currentMs / totalMs) * 100))); await updateProgress(job.id, { phase: type === "PREVIEW" ? "PREVIEW_RENDER" : "ENCODING", message: `Rendering ${type.toLowerCase()}`, progress, currentMs: BigInt(Math.round(currentMs)), totalMs: BigInt(totalMs), speed: values.speed }); } };
  ffmpegProcess.stdout.on("data", chunk => { stdoutBuffer += chunk.toString(); const lines = stdoutBuffer.split("\n"); stdoutBuffer = lines.pop() ?? ""; for (const line of lines) void progressListener(line); });
  ffmpegProcess.stderr.on("data", chunk => { stderr += chunk.toString(); });
  await new Promise<void>((resolve, reject) => { ffmpegProcess.on("close", code => code === 0 ? resolve() : reject(new Error(`FFmpeg exited with code ${code}: ${stderr.slice(-2000)}`))); ffmpegProcess.on("error", reject); });
  runningProcesses.delete(job.id);
  await createOutput(project.id, job.id, "VIDEO", outputPath, "video/mp4", type === "PREVIEW");
...
async function processJob() {
  ...
  try {
    if (job.type === "DOWNLOAD") await processDownload(job);
    else await runFfmpegJob(job, job.type);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await updateProgress(job.id, { status: "FAILED", phase: "FAILED", message, error }, true);
    await logJobEvent(job.id, "ERROR", "Media job failed", { error: message, type: job.type });
  }
  return true;
}
...
async function processPublication() {
  ...
  if (!publication?.output) return false;
