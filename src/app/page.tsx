...
type Project = {
  ...
  source?: { type: string; originalName?: string | null; storagePath?: string | null } | null;
...
export default function HomePage() {
  ...
  const [title, setTitle] = useState(""); const [preacher, setPreacher] = useState("");
  ...
  async function createProject(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/projects", {
      ...
      body: JSON.stringify({ title, preacher, sourceUrl: sourceMode === "youtube" ? sourceUrl : undefined, segments }),
    });
    const data = await response.json();
    if (!response.ok) { setError(data.error ?? "Project creation failed"); return; }
    if (sourceMode === "upload") {
      if (!file) { setError("Choose a source file"); return; }
    ...
    setMessage(`Project ${data.id} queued for generation.`); await refresh();
  }
  ...
  return <main>
    ...
    <form onSubmit={createProject}>
      ...
      <section className="card"><h2>Source</h2>
        <div><label><input type="radio" checked={sourceMode === "upload"} onChange={() => setSourceMode("upload")} /> Upload file</label>{" "}<label><input type="radio" checked={sourceMode === "youtube"} onChange={() => setSourceMode("youtube")} /> YouTube URL</label></div>
        {sourceMode === "upload" ? <input type="file" accept="video/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} required /> : <input type="url" value={sourceUrl} onChange={(e) => setSourceUrl(e.target.value)} placeholder="https://www.youtube.com/watch?v=..." required />}
      </section>
      <section className="card"><h2>Sections</h2><p className="muted">Times are seconds from the beginning of the source recording.</p><div className="grid two">
    ...
    <section className="card"><h2>Projects</h2>{projects.length === 0 ? <p className="muted">No projects yet.</p> : projects.map((project) => {
      ...
      return <article key={project.id} className="project-row"><strong>{project.title}</strong><span>{project.preacher ?? ""}</span><span>{job ? `${job.status} (${job.progress}%)` : "No job"}</span>{project.outputs?.map((output) => <a key={output.id} href={`/api/outputs/${output.id}`}>{output.type === "VIDEO" ? "Download video" : "Download thumbnail"}</a>)}</article>;
    })}</section>
  </main>;
}
