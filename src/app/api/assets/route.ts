...
const metadataSchema = z.object({ assetKey: z.string().trim().min(1).max(64), type: z.enum(["OVERLAY", "BACKGROUND", "LOGO", "FONT"]).default("OVERLAY"), folderId: z.string().nullable().optional() });

function serialize(asset: any) { return { id: asset.id, assetKey: asset.assetKey, type: asset.type, mimeType: asset.mimeType, width: asset.width, height: asset.height, hasAlpha: asset.hasAlpha, sizeBytes: asset.sizeBytes.toString(), contentHash: asset.contentHash, folderId: asset.folderId ?? null, projectCount: asset.projects?.length ?? 0, createdAt: asset.createdAt?.toISOString?.() }; }
...
export async function POST(request: Request) {
  try {
    ...
    const metadata = metadataSchema.safeParse({ assetKey: form.get("assetKey"), type: form.get("type") || undefined, folderId: form.get("folderId") || null });
  ...
  } catch (error) { console.error("Asset library upload error:", error); return NextResponse.json({ error: "Asset upload failed" }, { status: 500 }); }
}
