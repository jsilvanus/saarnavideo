...
import { formatSrt } from "@/lib/captions";
...
export async function GET(_request: Request, context: { params: Promise<{ sourceId: string }> }) {
  ...
  const body = formatSrt(segments);
  const filename = captionsFilename(source, "srt");
  return new NextResponse(body, {
    status: 200,
    headers: { "Content-Type": "application/x-subrip; charset=utf-8", "Content-Disposition": `attachment; filename="${filename}"` },
  });
}
